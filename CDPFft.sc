...
// Create the window
win = Window.new("CDP", Rect(20, 200, 560, 450 + (process.params.size * 50)), scroll: true);
win.front;
...
analyseButton.action = {arg state;
	...
	input = current.path;
	output = path +/+ "input/"++ soundMenu.item.replace(".wav", " ").toLower.replace(" ", "") ++".ana";
...
processButton.action = {arg state;
	var input, output, params, current;
	current = inputSounds[soundMenu.value];
	input =  path +/+ "input/"++ soundMenu.item.replace(".wav", " ").toLower.replace(" ", "") ++".ana";
	output = path +/+ "output/"++ (soundMenu.item.replace(".wav", " ") ++ "_" ++ process.name).toLower.replace(" ", "") ++".ana";
	params = "";
...
synthesizeButton.action = {arg state;
	var input, output, params, current;
	current = inputSounds[soundMenu.value];
	input = path +/+ "output/"++ (soundMenu.item.replace(".wav", " ") ++ "_" ++ process.name).toLower.replace(" ", "") ++".ana";
